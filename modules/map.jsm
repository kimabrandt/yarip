...
YaripMap.prototype.get = function(pageName) {
    if (pageName in this.obj) {
        return this.obj[pageName];
//    } else {
//        yarip.logMessage(LOG_WARNING, new Error("YaripMap.get(" + pageName + "): undefined"));
    }
}
...
YaripMap.prototype.generateXml = function(exporting) {
    ...
    this.tree.traverse(function(node) {
        if (node.value) {
            r += node.value.generateXml();
        }
    });
    r += "</yarip>";
    return r;
}
YaripMap.prototype.toJSON = function() {
    var r = {
        "version": VERSION,
        "pages": []
    };
    this.tree.traverse(function(node) {
        if (node.value) {
            r.pages.push(node.value);
        }
    });
    return r;
}
//YaripMap.prototype.fromJSON = function(mapObj) {
//    var map = new YaripMap();
//    for each (var pageObj in mapObj) {
//        map.add(YaripPage.fromJSON(pageObj));
//    }
//    return map;
//}
